...
const DetalleNoodle = () => {
  ...
  const menuNoodles = noodles.find((noodle) => noodle.titulo === noodleId)
  ...
  const handlePedido = () => {
    alert(`Pedido realizado:
      Sopa: ${menuNoodles.nombre}
      Cantidad: ${cantidad}
      Precio Total: S/. ${menuNoodles.precio * cantidad}`);
  ...
  const precioTotal = menuNoodles.precio * cantidad;
      ...
      <div className='flex flex-row w-fit gap-10 content-center h-fit border-marca-red border-4 items-start bg-white rounded-2xl shadow-2xl'>
        <div>
          <img
            ...
            src={menuNoodles.imagen}
            alt={menuNoodles.titulo}
          />
        </div>
        <div className='flex flex-col justify-between w-[30rem] h-4/5 p-9'>
          <div>
            <p className='text-sm font-light italic'>{menuNoodles.categoria}</p>
            <h3 className='text-marca-red font-bold text-5xl pb-7'>{menuNoodles.nombre}</h3>
            <p className='text-lg pb-10'>
              <span className='text-sm font-light italic'>ingrdientes</span><br/>
              {menuNoodles.ingredientes}
            </p>
            <p className='font-semibold text-4xl'>
              <span className='text-sm font-light italic'>precio unitario <br/></span>
              S/. {menuNoodles.precio}
              </p>
          </div>
          <div className='mt-5'>
